...
namespace FRED
{
    public class SeriesObservations : Request
    {
        public readonly List<string> Date = new List<string>();
        public readonly List<string> Value = new List<string>();

        public SeriesObservations(string api_key, string series_id) : base("http://api.stlouisfed.org/fred/series/observations?series_id=" + series_id + "&api_key=" + api_key) { }
        public SeriesObservations() { }
                ...
                case "observations":
                    foreach (System.Xml.XmlNode xmlNode in xmlDocument.DocumentElement.ChildNodes)
                    {
                        if (xmlNode is System.Xml.XmlElement)
                        {
                            ...
                            {
                                case "observation":
                                    Date.Add(xmlElement.Attributes["date"].Value);
                                    Value.Add(xmlElement.Attributes["value"].Value);
                                    break;
                                default:
